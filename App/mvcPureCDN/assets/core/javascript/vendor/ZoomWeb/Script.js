...
(function(a) {
 a.fn.wow_zoom = function(b) {
  ...
  a.each(this, function() {
   var aG = a(this);
   if (aG.is("ul") && aG.children("li").length && aG.find("img.wow_zoom_source_image").length) {
    ...
    aG.addClass("wow_zoom").show();
    var w = aG.children("li").addClass("wow_zoom_thumb");
    w.first().show().addClass("wow_zoom_thumb_active");
    var q = w.length,
     aJ = c.autoplay;
    if (q < 2) {
     aJ = false
    ...
    if (c.align === "right") {
     aG.addClass("wow_zoom_right")
    }
    a.each(w, function(a1) {
     a1 += 1;
     var a4 = a(this),
      j = a4.find(".wow_zoom_thumb_image").removeAttr("alt").show(),
      a3 = a4.find(".wow_zoom_source_image"),
      a2 = a4.find("a");
     a4.data("id", a1).addClass("thumb_" + a1);
     if (!j.length && a3.length) {
      a4.prepend('<img class="wow_zoom_thumb_image" src="' + a3.attr("src") + '" />')
     } else {
      if (!j.length && !a3.length) {
       a4.remove()
      }
     ...
     if (a2.length) {
      a4.find(".wow_zoom_thumb_image").data("anchor", a2.attr("href"))
     }
    });
    var av = w.find(".wow_zoom_thumb_image").css({
    ...
    var aO = a('<li class="wow_zoom_magnifier"><div><img /></div></li>').appendTo(aG),
     aa = aO.children("div"),
     h = aa.children("img");
    var E = a('<li class="wow_zoom_icon">&nbsp;</li>').appendTo(aG);
    if (c.show_icon) {
     E.show()
    }
    var r;
    if (c.show_hint) {
     r = a('<li class="wow_zoom_hint">&nbsp;</li>').appendTo(aG).show()
    }
    var K, s = c.zoom_element;
    if (s !== "auto" && s && a(s).length) {
     K = a(s).addClass("wow_zoom_zoom_area").html('<div><img class="wow_zoom_zoom_img" /></div>')
    } else {
     s = "auto";
     K = a('<li class="wow_zoom_zoom_area"><div><img class="wow_zoom_zoom_img" /></div></li>').appendTo(aG)
    }
    var W = K.children("div"),
     an;
    if (ao) {
     an = a('<img class="wow_zoom_zoom_preview" />').css({
      width: c.source_image_width,
      height: c.source_image_height,
      opacity: 0.3
     }).prependTo(W).show()
    }
    var aB = W.children(".wow_zoom_zoom_img").css({
     width: c.source_image_width,
     height: c.source_image_height
    });
    var az;
    if (c.show_descriptions) {
     az = a('<div class="wow_zoom_description' + ((c.right_to_left) ? " rtl" : "") + '"></div>').prependTo(K)
    }
    var aQ, l, aV, u, y, aj = c.small_thumbs;
    if (q > 1 || !c.smallthumb_hide_single) {
     aQ = a('<li class="wow_zoom_small_thumbs"><span class="p"><span class="c-c al-c"><i class="flaticon-previous11"></i></span></span><ul></ul><span class="n"><span class="c-c al-c"><i class="flaticon-next15"></i></span></span></li>').appendTo(aG);
     l = aQ.children("ul");
     a.each(av, function() {
      var i = a(this);
      Q = i.data("src");
      I = i.parents(".wow_zoom_thumb").data("id");
      a('<li><img class="wow_zoom_small_thumb" src="' + Q + '" /></li>').data("thumb_id", I).appendTo(l)
     });
     ...
     if (q > aj) {
      if (c.show_begin_end_smallthumb) {
       ...
       I = w.eq(q - 1).data("id");
       a('<li class="wow_zoom_smallthumb_first wow_zoom_smallthumb_navtoend"><img class="wow_zoom_small_thumb" src="' + Q + '" /></li>').data("src", Q).data("thumb_id", I).css({
        opacity: c.smallthumb_inactive_opacity
       }).prependTo(l);
       Q = av.eq(0).data("src");
       I = w.eq(0).data("id");
       a('<li class="wow_zoom_smallthumb_navtostart"><img class="wow_zoom_small_thumb" src="' + Q + '" /></li>').data("src", Q).data("thumb_id", I).css({
        opacity: c.smallthumb_inactive_opacity
       }).appendTo(l);
       aV = l.children("li");
       aV.eq(1).addClass("wow_zoom_smallthumb_active").css({
        opacity: 1
       })
      } else {
       aV.eq(0).addClass("wow_zoom_smallthumb_first wow_zoom_smallthumb_active").css({
        opacity: 1
       })
      }
      aV.eq(aj - 1).addClass("wow_zoom_smallthumb_last")
     } else {
      aV.eq(0).addClass("wow_zoom_smallthumb_active").css({
       opacity: 1
      })
     }
    ...
    var ay = w.first().find(".wow_zoom_thumb_image"),
     ap = w.first().find(".wow_zoom_source_image");
    if (c.magnifier_invert) {
    ...
    var g = function(a3, a1) {
     var j, a2, i = aG.find(".wow_zoom_smallthumb_active").removeClass("wow_zoom_smallthumb_active");
     a3.addClass("wow_zoom_smallthumb_active");
     aO.stop().hide();
     K.stop().hide();
     if (!a1) {
      ai = true;
     ...
     aG.find(".wow_zoom_thumb_active").removeClass("wow_zoom_thumb_active").stop().animate({
      opacity: 0
     }, c.speed, function() {
      a(this).hide()
     });
     j = w.filter(".thumb_" + a3.data("thumb_id")).addClass("wow_zoom_thumb_active").show().stop().css({
      opacity: 0
     }).animate({
      opacity: 1
     }, c.speed);
     ay = j.find(".wow_zoom_thumb_image");
     ap = j.find(".wow_zoom_source_image");
     if (c.magnifier_invert) {
    ...
    var G = function(a2, j, i, a1) {
     a.each(aV, function() {
      ...
      a4.animate(a3, aZ, "swing", function() {
       if (ai) {
        a1.addClass("wow_zoom_smallthumb_active");
        ai = false
       }
      })
     });
    ...
    var L = function() {
     var i;
     if (c.magnifier_invert) {
      aG.find(".wow_zoom_thumb_active").mouseleave()
     }
     if (!c.right_to_left) {
      i = aG.find(".wow_zoom_smallthumb_active").prev();
      if (!i.length) {
       if (q > aj) {
        F()
       } else {
      ...
      i = aG.find(".wow_zoom_smallthumb_active").next();
      if (!i.length) {
       if (q > aj) {
        ac()
       } else {
    ...
    var n = function(a2) {
     ...
     var a6 = aV.eq(a2);
     if (a6.length && !ai) {
      var a5 = aG.find(".wow_zoom_smallthumb_active"),
       a1 = a5.data("id") - 1,
       j;
      if (a1 > a2) {
       z = a1 - a2;
       var a3 = aG.find(".wow_zoom_smallthumb_first"),
        a7 = a3.data("id");
       if (a2 < a7) {
        j = a1 - a7;
        z = z - j;
       ...
       if (a1 < a2) {
        z = a2 - a1;
        var a4 = aG.find(".wow_zoom_smallthumb_last"),
         i = a4.data("id") - 1;
        if (a2 >= i) {
         j = i - a1 - 1;
         z = z - j;
    ...
    var aI = function(i) {
     ...
     if (typeof wow_zoom_click_callback === "function") {
      wow_zoom_click_callback(i, aR);
      return false
     }
     return true
    };
    var ah = function(i) {
     if (c.change_callback(i, aR)) {
      if (typeof wow_zoom_change_callback === "function") {
       wow_zoom_change_callback(i, aR)
      }
     }
    };
    ...
    function aP(a2) {
     z = (z) ? z - 1 : 0;
     ai = true;
     var a1 = aG.find(".wow_zoom_smallthumb_first").removeClass("wow_zoom_smallthumb_first");
     var i = aG.find(".wow_zoom_smallthumb_last").removeClass("wow_zoom_smallthumb_last");
     var a3, j, a5, a4;
     if (a2 === "left") {
      a3 = a1.prev().addClass("wow_zoom_smallthumb_first");
      j = i.prev().addClass("wow_zoom_smallthumb_last");
      a5 = a1
     } else {
      a3 = a1.next().addClass("wow_zoom_smallthumb_first");
      j = i.next().addClass("wow_zoom_smallthumb_last");
      a5 = i
     }
     if (z) {
      if (a2 === "left") {
    ...
    function m(a5) {
     ...
     var a2, j, a4;
     if (a5 === "end") {
      a2 = aV.eq(y - aj).addClass("wow_zoom_smallthumb_first");
      j = aV.eq(y - 1).addClass("wow_zoom_smallthumb_last");
      a4 = j;
      if (j.hasClass("wow_zoom_smallthumb_navtostart")) {
       a4 = j.prev()
      }
     } else {
      a2 = aV.eq(0).addClass("wow_zoom_smallthumb_first");
      j = aV.eq(aj - 1).addClass("wow_zoom_smallthumb_last");
      a4 = a2;
      if (a2.hasClass("wow_zoom_smallthumb_navtoend")) {
       a4 = a2.next()
      }
     }
    ...
    if (q > 1 || !c.smallthumb_hide_single) {
     aV.click(function() {
      var a7 = a(this),
       ...
       a5 = false,
       a2, a8, a4, a6, a1;
      if (!a7.hasClass("wow_zoom_smallthumb_active") && (!ai || z)) {
       if (a7.hasClass("wow_zoom_smallthumb_first") && a7.prev().length) {
        aP("left")
       } else {
        if (a7.hasClass("wow_zoom_smallthumb_navtoend")) {
         F()
        } else {
         if (a7.hasClass("wow_zoom_smallthumb_last") && a7.next().length) {
          aP("right")
         } else {
          if (a7.hasClass("wow_zoom_smallthumb_navtostart")) {
           ac()
          } else {
...
$(document).ready(function() {
 ...
 $(document).on('click', '.wow_zoom_small_thumbs > .p', function() {

  wow_zoom_previous();


 });
 $(document).on('click', '.wow_zoom_small_thumbs > .n', function() {
  wow_zoom_next();
