; (function(W){
  ...
  var frombody=W.T.dashboard_brands.productedit;
var formLogic =function() {
     var rv = ['brand_name', 'pid', 'sid', 'has_varient'],
   ...
   var category = Array(),searchword= [''],j=0,i=0;
        $(':hidden.tokenh_input').each(function () {
            if ($(this).attr('name') == 'companycategory') {
...
//form data
    ...
    var ch ='<form name="brand_one"  data-junction="brand_one" onsubmit="return false"> </form>';
   
          W.U.JunctionAdd(W.A.page.AppId,'brand_one',function(){
  ...
  var frombody=W.T.dashboard_brands.productcopy;
var formLogic =function() {
...
var frombody=W.T.dashboard_brands.inventryFormBody;
var DeleteBody=W.T.dashboard_brands.inventryDeleteBody;
...
var formLogic =function() {
  var formData=this.formData; 
     var rv = ['currency', 'unitsystem', 'pid', 'vid', 'sell_price', 'compare_price', 'unique_identity'],
      f_value = W.F.walk_way_all(rv, this.formname),
      error=7, alert_mes = [];
...
if(TriggerData[4]=="edit"){
 ...
 if(TriggerData[4]=="delete"){
   ...
   // W.U.Pager.DirectPagerbtnOnclick(''+ W.I.dp+':inventorylistedit:sp:'+ProductData.id+'');  
...
if(TriggerData[4]=="delete"){
 ...
 ch ='<form name="addbrandinventory"  data-junction="addbrandinventory" onsubmit="return false"> </form>';
   
          W.U.JunctionAdd(W.A.page.AppId,'addbrandinventory',function(){
 ...
 return W.T.dashboard_brands.inventoryList(block);
   }
   ...
   var ProductData=block.objectdata; 
    if(!W.U.isOK(ProductData.spfl_ischanged)){
     ...
     ch+='<div class="block m_b10 m_t10 _bdy"><a href="javascript:void(0);" class="btn btn-block"  data-pagerbtn="'+ W.I.dp+':specificationedit:db:'+ProductData.id+':addnew:0:specificationList"  ><span class="vl-sp fw-b">Add New Specifications</span><span class="vl-sp right">'+W.T.SVG('nextarrow',24,'#00768D')+'</span></a></div>';

        if(spfl.length>0){
   ch+='<div class="block m_b10 m_t10 _bdy "><div class="block spfc m_b10 bs-1">';
            for(var i in spfl){
     
        ch+='<div class="block _bdy h po-re "> <div class="block ov-hi">'+spfl[i].heading+'</div><div class="block ov-hi"> <a href="javascript:void(0);" class="span t_t"  data-pagerbtn="'+ W.I.dp+':specificationedit:db:'+ProductData.id+':edit:'+i +':specificationList"  style="margin:0px 20px;" >Edit&nbsp;&nbsp;</a><a href="javascript:void(0);"   data-pagerbtn="'+ W.I.dp+':specificationedit:db:'+ProductData.id+':delete:'+i +':specificationList"  class="span t_t" >&nbsp;&nbsp;delete</a></div> </div>';
...
spflBHandler.prototype.submit=function(){
  ...
  var  formData={
    ...
    ProductData.spfl.splice(id, 1);
          ...
          W.U.intentdata.add('db.'+ProductData.id,ProductData);
...
//-->>
 ...
 ch='<div class="block "><a class="hide" href="javascript:void(0);" data-btnid="poBack"   data-openbtn="productOptionsInner"  ></a>';
   ...
   //   option.push({name:'text_52',des:'help_68',pageid:"productSimilarProduct"});
    ...
    for(var  q in option){
      nav+='<div class="li bs-1 "> <a class="block _bdy" href="javascript:void(0);" data-pagerbtn="'+ W.I.dp+':'+option[q].pageid+':db:'+ProductData.id+':edit:0:blockFront"  > <span class="vl-sp fw-b">'+option[q].name+'</span> <span class="vl-sp right">'+W.T.SVG('nextarrow',18,'#1274c0')+'</span> </a> <span class="block fg_4 fs-italic fs11 _bdy">'+option[q].des+'</span> </div>';  
...
return ch;
 ...
 Handler.prototype.Submit=function(){
     ...
     var form = 'AddbrandFilter',help=W.U('[data-help="' + form + '"]')[0];
...
var name=this.data.name;
 ...
 W.U.Pager.addblockdata({  name:'productedit', htmlStr:productedit});
 W.U.Pager.addblockdata({  name:'productcopy', htmlStr:productcopy});
...
W.U.Pager.addblockdata({  name:'inventorylistedit', htmlStr:inventorylistedit});
 ...
 var mainBlock=W.U.Rander(W.T.dashboard_brands.Layout());
...
W.U.dashboard_brands={init:init};
