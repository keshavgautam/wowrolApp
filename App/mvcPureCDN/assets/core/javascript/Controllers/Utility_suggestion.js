...
; (function(W){
 ...
 this.wrapdata = W.U.extend(Handler.Default, wrapdata);
   ...
   this.suggestionDropDown=  W.U.DropDown(this.dom.wrapChild,{
       type:'suggestion',
       placement:'bottom', 
       trigger:'',
       width:function(){
        ...
        Dropdown:this.dom.down});//initialization


        // W.U.console(this);
        // W.U.console(typeof (this.wrapdata.hover));
        if (this.wrapdata.hover) {
    ...
    Handler.prototype.oninput = function () {
        ...
        var formData = {
            ...
            f_value: {
                inputval: val,
                selected: selected,
               ifo: JSON.stringify(this.wrapdata.ifo),
    ...
    Handler.prototype.getselected = function () {
       ...
       var hiddeninput = W.U('[type="hidden"]',this.dom.up);
        for (var q = 0, i = 0; q < hiddeninput.length; q++) {
             var val = $(hiddeninput[q]).val();
             if(W.U.isOK(val)){
            switch(this.wrapdata.token){
                case 'smartchips':
                   var TermData= W.U.intentdata.get(val);
                   if(W.U.isOK(val)){
                     selected[i] = TermData.id;
                    i++;     
                   }
                break;
                default:
        ...
        var DropdownModule = {
            lia: function (x) {
                ...
                for (var q in x) {
                    ...
                    ch += '<li  class="li _B-gray"  ><a class="block _bdy"  href="javascript:void(0);"  data-suggestionli=\'' + JSON.stringify(x[q].li_data) + '\' >' + W.U.highlight(x[q].name, input_value) + '</a></li>';
                }
...
var saveingId='suggestion.'+W.U.uId();
 W.U.intentdata.add(saveingId,li_data);
        ...
        switch (x.Module) {
            ...
            break;
               case 'dashboardmenu':
              list += DropdownModule.dashboardmenu(x.Dropdown);
            break;
       ...
       W.U.Setview(this.suggestionDropDown.Dropdown, parselist, 'html');
 ...
 //     W.U.console(this);
        var token = suggestion.maketoken({ 
        type: suggestion.wrapdata.token,
            data: {
...
allclose[p].onclick=suggestion.onclose.bind({ suggestion: suggestion, self: allclose[p] });
      ...
      // suggestion.dom.down.style.display = "none";
       suggestion.suggestionDropDown.hide();
      ...
      var onselect= suggestion.wrapdata.onselectCallback;
      onselect.bind({suggestion:suggestion,data:data})();
...
var data456 = W.U.extend(Handler.Default,suggestion.wrapdata);
            ...
            var template = {
                ...
                smartchips: function (x) {
                    ...
                    ch += '<div class="li"><div class="token"> <span>' + x.name + '</span> <span class="sclose s_tclose"  data-suggestionClose=\'' + JSON.stringify({
                        ...
                        token: suggestion.wrapdata.token,
                        placeholder: suggestion.wrapdata.placeholder
                    }) + '\' ></span> <input class="tokenh_input" type="hidden"  name="' + suggestion.wrapdata.name + '" value="'+x.li_data+'" > </div></div>';
...
return ch;
            ...
            switch (x.type) {
                case 'chips':
                    ...
                    break;
               case 'smartchips':
                    token += template.smartchips(x.data);
                    break;
            }
