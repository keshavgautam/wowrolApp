...
; (function(W){
  ...
  var m={};
 //---
 W.I.LoaderInterval = 0; //reference to inter val
...
//ret= true;
  ...
  W.U.console('smartSetup started');  
...
for (var i in resListName) {
     ...
     return newres;
      ...
      function  ResLoader (res){
          ...
          switch(this.res.type){
                   ...
                   url+= this.res.path+this.res.name+'.js';
                   break;
                   case 'style':
                    url+= this.res.path+this.res.name+'.css';
                   break;
               }
 ...
 ResAddALL();
   ...
   checkondepend(['vendorjs'],insertres);
      break;
      case 'bootstrapjs':
    checkondepend(['vendorjs','commonjs'],insertres);
      break;
       case 'appjs':
    checkondepend(['vendorjs','bootstrapjs','commonjs'],insertres);
      break;
      default:
      insertres();
  }
...
var resList =ResCheck.bind(this.res)();
  ...
  W.U.console('normalSetup started');  
 ...
 url=this.res.url;  
  ...
  //---------------------------------
        function checkloading(){   
            ...
            if(tries<100){
            if(W.I.backinit&&(typeof   W.U.SetAppView!='undefined')){
            clearInterval( W.I.LoaderInterval); //clear if any previous
           
          
      setTimeout(W.U.queue.Run,500);
       
            W.U.console(' page loaded');  
            }else{
                W.U.console(' page loading checking');  
            }
           
            }else{
                   W.U.console('unable to load page');   
...
W.U.SetAppView(App); 
   ...
   clearInterval( W.I.LoaderInterval); //clear if any previous
    W.I.LoaderInterval = setInterval(checkloading, 1000); 
 ...
 W.C.loader={
            init: function(){
                var data= wowrol.A.page;
                  resurl= data.resurl;
 W.U.Page=document.getElementById('page');;
   var bro=W.U.browser;
   if(bro.msie6||bro.msie7||bro.msie8||bro.msie9||bro.safari_mobile){
     
loadscript( resurl+'core/javascript/polyfill.6a033088d25b39389b914930a16b6151.js','shim', 'shim',function(){
