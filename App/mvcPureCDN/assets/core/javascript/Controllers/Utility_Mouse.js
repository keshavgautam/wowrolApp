...
; (function (W) {
    ...
    * MIT License
    start
    Object {x: 406, y: 27, time: 1461468310512}
    diff
    Object {x: -202, y: -5, time: 505}
    speed
    Object {x: 0, y: 0}


    */
    function Mouse(_this, options) {
        ...
        ///Ensureing touch end event
        var TimeoutTouchEndId = null;
        ...
        function tMove(event) {
            ...
            window.clearTimeout(TimeoutTouchEndId);
            var data = { event: event, start: start, diff: diff, speed: speed };
        /*    TimeoutTouchEndId = window.setTimeout(function () {
                W.U.console('TimeoutTouchEndId');
                W.U.console(data.diff);
                //o.end(data.event, data.start, data.diff, data.speed);
            }, 2000);*/
        ...
        function init() {
            ...
            if( o.clicksAllowed){//add by keshav
               listeners.push(addEvent(_this, 'click', function (event) {
                o.clicksAllowed ? o.click(event) : preventDefault(event);
            }));  
            }
