...
var t={
 ...
 for(var q in suPD){
     var info=W.U.Browsing.GetPbankData(suPD[q],'');
  if(W.U.isOK(info)){
    var CartCardsetting={quentity:0,closebtn:true,varientid:'',type:'suggestion',role:SBData.role,currencysymbol:SBData.currencydata.symbol};
 ...
 if(product_card.length>0){
  ch+='<div class="block m_b10" ><div class="block  bg_0 " >'+product_card.join('')+'</div></div>'; 
     ...
     ch+='<div class="block _bdy bg_0 m_b10"> <p class="al-c">Nothing Suggestioned yet.</p> </div>'; 
    }
...
function Layout(block){
 ...
 mid+='<div class="block"  data-nodeid="checkinPlatformSuggestionWalkway'+SBData.checkIn_id+'"></div>';

  switch(W.I.initType){
     case 0:
     
ch+='<div class="block  bs-1"   >'+W.T.BottomFixWrap(header,mid,foot,'checkinPlatformSuggestion'+SBData.checkIn_id,margintop)+'</div>'; 
     break;  
     case 1:
  ch+='<div class="block  bs-1"   >'+W.T.BottomFixWrap(header,mid,foot,'checkinPlatformSuggestion'+SBData.checkIn_id,margintop)+'</div>';    
     break; 
