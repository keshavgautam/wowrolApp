...
//  W.U.console('productSliderchange');
       //         W.U.console(data);
var smallimages,bigimages,
selected=data.selected,
pvL=data.data.pvL;
  //var varientdata= (pvL.indexOf(selected)>0)?pvL[selected]:pvL[0];
     smallimages= W.U.ProductImage(pvL,'varientsmallimage',selected);
     bigimages= W.U.ProductImage(pvL,'varientbigimages',selected);
    // varientdata.webimages.concat(varientdata.mainimages);
     //bigimages=varientdata.webimages.concat(varientdata.mainimages);
...
ch+='<div class="block ul   m_b10" >'+b.phi(x)+b.des(x)+b.spf(x)+'</div>';//
ch+=b.upsell(x);
//ch+=b. declmair(x);
