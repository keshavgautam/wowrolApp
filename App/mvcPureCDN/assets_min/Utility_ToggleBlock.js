...
function ToggleBlock() {
  ...
  // W.U.console(this.we.blockListByName[id]);
  ...
  if( W.U.browser.opera_mini||W.U.browser.opera_mobile){
    ...
    Handler.prototype.update = function () {
        ...
        // W.U.console(ret); // W.U.console(this);
        var tb = ret.tb;
...
if(W.U.feature.pushState){
  // history.pushState('', "", updateUrl); 
    ...
    Handler.prototype.outer = function (e) {
        ...
        var id = e.id;
      //  W.U.console(e); W.U.console(this);
        switch (this.type) {
...
//trigger event from out side to swict block

W.U.SwitchBlock=function(action,id){
     var event = jQuery.Event(action);
     event.id=id;
   $(W.U.id('block.' + id)).parent().triggerHandler(event);   
